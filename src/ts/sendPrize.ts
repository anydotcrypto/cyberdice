...
async function sendPrize(
   ...
   * using etherjs.
   */
  const callData = cyberDiceCon.interface.functions.sendPrize.encode([]);
   ...
   */
  await watchRelayTx(relayTx, user, provider);

  const balance = await provider.getBalance(user.address);
  console.log(
    "New balance of " + user.address + " is  " + formatEther(balance)
  );
}

/**
 * Sends prize to the winner
 */
(async () => {
  ...
  if (anySenderBalance.lt(parseEther("0.009"))) {
    if (ethereumBalance.gte(parseEther("0.01"))) {
      ...
      console.log("Your ethereum balance is " + formatEther(anySenderBalance));
  ...
  console.log("Balance on any.sender: " + formatEther(anySenderBalance));

  const winner = await cyberDiceCon.winner();
  console.log("Winner: " + winner);

  if (winner !== user.address) {
    console.log(
      "Sorry, you are not the winner :( Thank you for participating. We hope it was a lot of fun!"
    );
    process.exit(0);
  }

  const balance = await provider.getBalance(winner);
  ...
  await sendPrize(user, provider, cyberDiceCon);
})().catch((e) => {
  console.log(e);
